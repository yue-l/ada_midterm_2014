...
public class UniqueQueue<E> implements Queue<E>{
		...
		private int rear;  // index of the tail of queue. This index should point to the position where we would add a new element to
		...
		/**
		 * A utility method for expanding the capacity of the current array
		 * The resulting array will be twice in length
		 */
		private void expandCapacity(){
            ...
            // After copying, the front of the queue is at index 0, the rear at index size
			for(int i=0;i<temp.length;i++){
				data[i]=temp[i];
			}
			front=0;
	     ...
	     * If value is already in the queue, delete it before re-entering at the tail
	     *
	     */
	    public void enqueue(E value){
			// Checking if the array has been completely filled 
            // by elements of the queue
            // If all positions are taken, then we need to expand the capacity of the array
		...
		public static void main(String[] args){
			...
			while(!q.empty()){
				System.out.println("Size: "+q.size()+" Front element: "+(String)q.dequeue());
			}
			System.out.println("");
